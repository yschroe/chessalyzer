...
Two things to notice: you **create a tracker instance** (`tileTracker()`) and hand it over, and after the call you **read your results from that same instance**. For a deeper look check out [Trackers](/docs/trackers).
...
<Callout type="info" title="Your performance numbers will differ">
    ...
    of games. Chessalyzer is carefully tuned for performance by utilizing
    [Multithreading](/docs/multithreading) and other techniques.
</Callout>

<Callout type="info" title="Multi-run analyses">
    The `runs` array has one entry per analysis pass and since we only ran one analysis cohort
    there's exactly one entry here. See [Comparing groups](/docs/comparing-analyses) for how to run
